...
export default async function CategoryPage({ params }: Props) {
  ...
  // Get subcategories - only if this is a parent category (like "Food" or "Plastics")
  // Parent categories are: "food" and "plastics"
  // All others are leaf categories and should only show products
  const isParentCategory = cat && ['food', 'plastics'].includes(cat.slug);
  const subcategories = isParentCategory ? await prisma.category.findMany({
    where: {
      section: cat.section,
      NOT: { slug: cat.slug },
    },
  ...
  // ALWAYS fetch products for this category
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      {subcategories.length > 0 && (
        <div className="mb-12">
          <p className="mb-4 text-slate-600 font-semibold">Subcategories:</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {subcategories.map((s) => (
              <Link key={s.id} href={`/category/${s.slug}`} className="border rounded-lg overflow-hidden group bg-white hover:shadow-lg transition-shadow">
                ...
                <div className="p-3 font-semibold text-center">{s.titleEn}</div>
              </Link>
            ))}
          </div>
        </div>
      ...
      {products.length > 0 && (
        <div>
          {subcategories.length > 0 && <p className="mb-4 text-slate-600 font-semibold">Products in {cat?.titleEn}:</p>}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {products.map((p) => {
              const displaySrc = p.imageUrl || subcatImage[p.category.slug] || '/images/logo.svg';
              return (
                <Link key={p.id} href={`/products/${p.slug}`} className="border rounded-lg overflow-hidden group hover:shadow-lg transition-shadow">
                  <Image src={displaySrc} alt={p.nameEn} width={400} height={400} className="w-full object-cover group-hover:scale-105 transition-transform" />
                  <div className="p-3">
                    <div className="font-semibold text-sm">{p.nameEn}</div>
      ...
      {/* Show message if no products and no subcategories */}
